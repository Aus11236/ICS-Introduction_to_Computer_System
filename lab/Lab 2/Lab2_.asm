...
SKIP1   ADD R4, R4, #1; move to the next character
        LDR R6, R4, #0
        BRz loo2; if we reached the end of the first string, then we move to the second
        BR loop1; otherwise, we continue the iteration

loo2    LDR R4, R1, #0; R4 points to the first character
...
SKIP02  AND R5, R5, #0
        ...
        BRn NO; if the value is already negative, they are not anagram
        STR R7, R5, #0; store the updated frequency count back to the frequency array
